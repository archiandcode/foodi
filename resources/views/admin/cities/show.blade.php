                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">
                            <a href="{{ route('admin.cities.index', ['country' => $country]) }}" class="btn btn-warning" title="{{ __('Назад') }}">
                                <i class="fa fa-arrow-left" aria-hidden="true"></i> {{ __('Назад') }}
                            </a>
                            <a href="{{ route('admin.cities.edit', [$country, $city]) }}" class="btn btn-primary" title="{{ __('Редактировать') }}">
                            ...
                            <form method="POST" action="{{ route('admin.cities.destroy', [$country, $city]) }}" accept-charset="UTF-8" style="display:inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" title="{{ __('Удалить') }}" onclick="return confirm('{{ __('Вы уверены, что хотите удалить этот город?') }}')">
